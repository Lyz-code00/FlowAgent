import { Activity, Bot, BrainCircuit, CheckCircle2, Github, Globe2, KeyRound, Network, PlugZap, Save, ShieldCheck } from "lucide-react";
  ...
  return (
    <>
      ...
      {!runtime || !agent || !github ? <LoadingBlock /> : <div className="config-grid">
        ...
        <section className="panel config-card"><div className="config-card__heading"><Activity size={21} /><div><h2>只读监控</h2><p>服务状态与故障证据</p></div><StatusBadge status={runtime.monitoring.health_services.length || runtime.monitoring.prometheus || runtime.monitoring.loki || runtime.monitoring.sentry ? "部分/全部已配置" : "未配置"} /></div><dl><div><dt>健康检查</dt><dd>{runtime.monitoring.health_services.length ? runtime.monitoring.health_services.join(", ") : "未配置"}</dd></div><div><dt>Prometheus / MQ</dt><dd>{runtime.monitoring.prometheus ? "已配置" : "未配置"}</dd></div><div><dt>Loki 日志</dt><dd>{runtime.monitoring.loki ? "已配置" : "未配置"}</dd></div><div><dt>Sentry</dt><dd>{runtime.monitoring.sentry ? "已配置" : "未配置"}</dd></div></dl></section>
        <section className="panel config-card"><div className="config-card__heading"><Globe2 size={21} /><div><h2>联网搜索</h2><p>公开网页搜索后端与失败保护</p></div><StatusBadge status={runtime.web_search.configured} /></div><dl><div><dt>搜索后端</dt><dd>{runtime.web_search.backend}</dd></div><div><dt>单次超时</dt><dd>{runtime.web_search.timeout_seconds} 秒</dd></div><div><dt>最大尝试</dt><dd>{runtime.web_search.max_attempts} 次</dd></div><div><dt>生产建议</dt><dd>{runtime.web_search.backend === "bocha" ? "国内 Bocha" : "建议配置 Bocha Key"}</dd></div></dl></section>
