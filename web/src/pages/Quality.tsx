  ...
  return (
    <>
      ...
      {loading ? <LoadingBlock /> : <>
        ...
        {benchmark?.context_cases && <section className="panel quality-detail"><div className="panel__header"><div><h2>上下文与安全验收</h2><p>30 组记忆证据、100 次重复投递和 30 组高风险确认码</p></div></div><div className="quality-latency-row"><div><span>上下文证据</span><strong>{benchmark.context_cases.filter((item) => item.passed).length} / {benchmark.context_cases.length}</strong><small>Issue/URL、历史约束、已确认待办</small></div><div><span>重复事件</span><strong>{benchmark.operation_cases?.duplicates ?? 0} / 99</strong><small>100 次投递，仅首条进入处理</small></div><div><span>确认码单次消费</span><strong>{benchmark.operation_cases?.confirmation_passed ?? 0} / {benchmark.operation_cases?.confirmation_total ?? 30}</strong><small>成功一次，复用必须拒绝</small></div></div></section>}
      </>}
    </>
  );
}
